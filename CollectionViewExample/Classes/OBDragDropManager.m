...
-(UIView *) findDropZoneHandlerInWindow:(UIWindow*)window atLocation:(CGPoint)locationInWindow
{
  ...
  if (!furthestView)
  {
    NSLog(@"OBDragDropManager findDropZoneHandlerInWindow: Furthest view is nil!");
    return nil;
  }
...
-(void) ovumSourceLongPressed:(OBLongPressDragDropGestureRecognizer*)recognizer
{
  ...
  if (recognizer.state == UIGestureRecognizerStateBegan)
  {
    ...
    recognizer.ovum.source = ovumSource;
	  ...
	  if ([ovumSource respondsToSelector:@selector(createDragRepresentationOfSourceView:inWindow:gestureRecognizer:)])
    {
		dragView = [ovumSource createDragRepresentationOfSourceView:recognizer.view inWindow:overlayWindow gestureRecognizer:recognizer];
    }
    else
    {
